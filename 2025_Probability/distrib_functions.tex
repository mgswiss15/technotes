...
\vskip 1em
\structure{What this means:}
\begin{itemize}
\item notation: $dP_X = p_X \, d\lambda$ or $dP_X(x) = p_X(x) \, dx$
\item Lebesgue measure is "weighted" by $p_X$ to give $P_X$
\item for any set $A$: $P_X(A) = \int_A p_X \, d\lambda = \int_A p_X(x) \, dx$
\end{itemize}

\vskip 1em
\structure{Key distinction:}
\begin{itemize}
\item density $p_X(x)$ can be evaluated at a point
\item but probability $P_X(\{x\}) = 0$ (single points have zero probability)
\end{itemize}

\vskip 0.5em
\alert{Density is NOT probability - it can be $> 1$!}
...
\item The notation $dP_X = p_X \, d\lambda$ means: infinitesimal probability = density times infinitesimal volume
\item This is analogous to: $df = f'(x) dx$ in calculus
\item Key distinction: $p_X(x)$ exists and can be evaluated, but $P_X(\{x\}) = 0$ for continuous distributions
\item Think of $p_X(x)$ as local concentration of probability, not probability itself
\item Density is what we actually compute and optimize in ML
\item Common mistake: thinking $p(x)$ must be $\leq 1$. Example: uniform on $[0, 0.5]$ has $p(x) = 2$
}

\begin{frame}{When does density exist?}

\structure{Absolute continuity condition:}
\begin{itemize}
\item $P_X$ is absolutely continuous w.r.t. Lebesgue measure $\lambda$
\item meaning: $\lambda(A) = 0 \Rightarrow P_X(A) = 0$
\item informally: zero volume implies zero probability
\end{itemize}

\vskip 1em
\structure{Radon-Nikodym theorem:}\\
If $P_X$ is absolutely continuous w.r.t. $\lambda$, then density $p_X = \frac{dP_X}{d\lambda}$ exists and is unique (almost everywhere)

\vskip 1em
\structure{When density does NOT exist:}
\begin{itemize}
\item discrete distributions (point masses)
\item mixed distributions (part discrete, part continuous)
\end{itemize}

\vskip 0.5em
\structure{In ML:} we work with continuous distributions $\Rightarrow$ densities exist
