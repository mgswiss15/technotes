...
\begin{itemize}
\item $P_X(B_\epsilon(x))$ = probability in small region around $x$
\item $\lambda(B_\epsilon(x))$ = volume of that region
\item ratio = probability per unit volume
\item limit as region shrinks to point = density at $x$
\end{itemize}
...
\note[enumerate]
{
\item This is the Radon-Nikodym derivative made concrete
\item In 1D: $B_\epsilon(x) = [x-\epsilon, x+\epsilon]$, $\lambda(B_\epsilon(x)) = 2\epsilon$
\item So $p_X(x) = \lim_{\epsilon \to 0} \frac{P_X([x-\epsilon, x+\epsilon])}{2\epsilon}$ - looks like derivative!
...
\structure{Condition: absolute continuity}\\
$P_X$ is absolutely continuous w.r.t. Lebesgue measure $\lambda$

\vskip 0.5em
Meaning: $\lambda(A) = 0 \Rightarrow P_X(A) = 0$ (zero volume $\Rightarrow$ zero probability)

\vskip 1em
\structure{Radon-Nikodym theorem:}\\
If $P_X$ is absolutely continuous w.r.t. $\lambda$, then density exists

\vskip 1em
\structure{When density does NOT exist:}
\begin{itemize}
\item discrete distributions (point masses have zero Lebesgue measure)
\item mixed distributions
\end{itemize}

\vskip 1em
\structure{In ML:} continuous distributions on $\mR^d$ $\Rightarrow$ densities exist

\end{frame}

\note[enumerate]
{
\item Absolute continuity is the key condition
\item Discrete distributions: probability on points, which have zero Lebesgue measure
\item For discrete: can define density w.r.t. counting measure instead
\item In generative modeling: we work with absolutely continuous distributions
\item This justifies working at the density level in code
}
