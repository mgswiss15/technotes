...
\vskip 0.5em
\structure{Why these density values?}
\begin{itemize}
\item $P_X([0.25, 0.75]) = \int_{0.25}^{0.75} p_X(x) \, dx = p_X \cdot 0.5 = 1 \cdot 0.5 = 0.5$
\item $P_Y([3.5, 4.5]) = \int_{3.5}^{4.5} p_Y(y) \, dy = p_Y \cdot 1 = \frac{1}{2} \cdot 1 = 0.5$
\item probability conserved: interval length doubles $\Rightarrow$ density halves
\end{itemize}
