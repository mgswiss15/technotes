...
\begin{scope}[shift={(-6,0)}]
    \node at (1.5,3) {$X \sim \text{Uniform}[0,1]$};
    ...
    \node[below] at (1.5,-0.8) {\small 50\% prob};
\end{scope}

% Arrow with transformation
\draw[->,very thick] (-2,1) -- (0,1) node[midway,above] {$g(x) = 2x+3$};

% RIGHT: Y ~ Uniform[3,5]
\begin{scope}[shift={(2,0)}]
    \node at (3,3) {$Y \sim \text{Uniform}[3,5]$};
    ...
    \node at (6,0.5) {$p_Y = \frac{1}{2}$};
    
    % Probability annotation
    \draw[<->,thick] (4.5,-0.8) -- (7.5,-0.8);
    \node[below] at (6,-0.8) {\small 50\% prob};
\end{scope}
\end{tikzpicture}
}
...
\begin{itemize}
\item interval $[0.25, 0.75] \to [3.5, 4.5]$: length $0.5 \to 1$ (doubled)
\item density: $p_X = 1 \to p_Y = \frac{1}{2}$ (halved to conserve probability!)
\end{itemize}
