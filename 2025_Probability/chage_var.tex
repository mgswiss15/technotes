...
\structure{Near point $x$, transformation $g$ is approximately linear:}
\[
...
\begin{itemize}
\item linear maps scale volumes by determinant
\item $J_g(x)$ is linear approximation at $x$
\item therefore: $|\det J_g(x)|$ = local volume scaling factor
\end{itemize}

\vskip 1em
\structure{Consequence for small region around $x$:}
\[
...
\note[enumerate]
{
\item Jacobian is the best linear approximation of $g$ near $x$
\item In 1D: $g(x + \Delta x) \approx g(x) + g'(x) \Delta x$ (tangent line)
