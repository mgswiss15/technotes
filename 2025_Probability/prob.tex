...
\item pre-image of $B \in T$: $\{X \in B\} = X^{-1}(B) = \{s \in S: X(s) \in B\} \in \salg$
\end{itemize}
...
\vskip 0.5em
\structure{Random variable $Y = g(X)$ with measureable func $g: T \to U$}
\begin{itemize}
\item for outcome $s \in S$, $Y$ takes value $y = g(x) = g(X(s)) \in U$ - realization of r.v. $Y$ 
\item pre-image of $C \in U$: $\{Y \in C\} = \{s \in S: X(s) \in g^{-1}(C)\} \in \salg$
\end{itemize}
...
\alert{push forward} of $P_X$ by $g$: $P_Y(C) = \mP(Y \in C) = \mP(\{s \in S: g(X(s)) \in C\}) = \mP(X \in B) = P_X(B)$ \\
\alert{pull back} of $P_Y$ by $g$: $P_X(B) = P_Y(\{u \in U: g^{1}(u) \in B\})$
...
\item $\mathcal{D} = \mathcal{N} \bigcup \mathcal{M} = \{A \in \mathscr{S} : \mP(A) = 0$ or $\mP(A) = 1\}$ \alert{essentially deterministic} events
\end{itemize}
...
% \draw[->,thick] (0,0.5) -- (2,0.5) node[midway,above] {$f: S \to T$};
% \draw[<-,thick] (0,-0.5) -- (2,-0.5) node[midway,above] {$f^{-1}: T \to S$};
% \node at (1,-1.5) {\color{carnelian}$\mu(f^{-1}(B)) = \nu(B)$};
% \end{scope}
...
%     % Preimage region f^{-1}(B): ALIGNED WITH GRID
%     % Rectangle from x = -1 to 0, y = -1 to 1
%     \fill[blue!25,opacity=0.5] (-0.5,-1) rectangle (1.5,1);
%     \draw[blue!60,thick] (-0.5,-1) rectangle (1.5,1);
%     \node[blue!80] at (-0.5,0) {$f^{-1}(B)$};
% \end{scope}
